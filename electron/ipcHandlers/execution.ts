...
export function registerExecutionHandlers(ipcMain: Electron.IpcMain, _context: IpcHandlerContext) {
    const codeCompletionSvc = () => AIService.getForProvider(pipelineService.getProviderFor('code_generation'));
        ...
        try {
            ...
            if (!report.passed) {
                ...
                for (repairAttempts = 1; repairAttempts <= 3; repairAttempts++) {
                    if (!codeCompletionSvc().isActive()) break;

                    const response = await codeCompletionSvc().chat([
                    ...
                    ], { temperature: 0.1, model: pipelineService.getModelFor('code_generation') }) as import('../services/AIService').ChatResponse;
